 ...
 * @比较重要
 * 1.修改边框 (根据需求瞄frame的边框)
 * 2.线也是在该自定义View里画 (可以实现移动向下的功能)
 * 3.背景色,线的颜色修改等等
 */
...
//    maskColor = resources.getColor(R.color.viewfinder_mask);
    ...
    public void onDraw(Canvas canvas) {
      ...
      */
            ...
            paint.setColor(laserColor);
            //计算线的起始位置
            int start = frame.top + INCREASE;
            //计算线终止设置的位置
            int end = frame.top + frame.height(); //frame.bottom()
            //每次增加的距离5
            if (start <= end) {
                INCREASE += 5;
            } else { //还原初始值
            ...
            //把原来的middle改成了start
            canvas.drawRect(frame.left + 2, start - 1, frame.right - 1, start + 2, paint);
